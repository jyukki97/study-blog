...
decision_guide:
  title: "REST와 gRPC, 전환 전에 나눠 볼 세 가지 상황"
  intro: "단일 벤치마크 결과가 아니라 호출 주체·병목·운영 계약을 함께 확인한 뒤, 작은 범위에서 검증 가능한 선택부터 시작합니다."
  cases:
    - badge: "REST 유지"
      title: "외부 공개 API와 브라우저 경험이 중심일 때"
      fit: "파트너 연동, 브라우저 디버깅, HTTP 캐시와 문서화 편의성이 중요한 서비스"
      watchouts: "REST를 유지해도 대용량 응답·연결 재사용·DB 병목은 별도로 측정해야 한다."
      next_step: "대표 엔드포인트의 p95·p99, 페이로드 크기, 캐시 적중률을 먼저 기준선으로 남긴다."
    - badge: "gRPC 우선 검증"
      title: "내부 고빈도 호출과 명확한 계약이 필요한 때"
      fit: "서비스 간 호출량이 많고 Proto 기반 타입 계약·스트리밍 이점이 운영 비용을 상쇄하는 환경"
      watchouts: "코드 생성, 하위 호환성, 관측 도구와 장애 대응 절차가 준비되지 않으면 전환 이득이 줄어든다."
      next_step: "한 개의 내부 호출을 대상으로 Proto 호환성 검사와 부하 테스트를 포함한 canary를 만든다."
    - badge: "혼합 전략"
      title: "외부와 내부의 요구가 다른 때"
      fit: "외부에는 이해하기 쉬운 HTTP API를 제공하고 내부에는 효율적인 서비스 간 통신이 필요한 환경"
      watchouts: "게이트웨이 변환 계층이 오류 의미·타임아웃·재시도 정책을 흐리게 만들지 않도록 계약을 명시해야 한다."
      next_step: "경계 API 한 곳에서 인증·오류 코드·관측 필드를 표준화한 뒤 점진적으로 확장한다."
learning_refs:
  - title: "gRPC 서비스 설계"
