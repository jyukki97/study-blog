...
summary: "Context Contract Registry는 모델 입력을 하나의 프롬프트 덩어리가 아니라 소유자, 신선도, 민감도, 허용 작업이 붙은 컨텍스트 조각 집합으로 관리하는 운영 레이어입니다. 핵심은 검색을 더 많이 붙이는 것이 아니라 어떤 문맥이 어떤 조건에서 들어와도 되는지를 계약처럼 명시하고 버전 관리하는 데 있습니다. 이 구조가 잡혀야 stale context, 무단 혼입, 과잉 토큰, 팀 간 충돌을 줄이면서 안전한 에이전트 입력 파이프라인을 만들 수 있습니다."
faqs:
  - question: "Context Contract Registry는 프롬프트 저장소와 어떻게 다른가요?"
    answer: "프롬프트 저장소가 지시문과 few-shot 예시 버전을 관리하는 데 가깝다면, Context Contract Registry는 모델에 들어가는 문서, 검색 결과, 메모, 로그, 정책 조각의 소유권과 허용 조건을 관리합니다. 즉 문장을 다듬는 저장소가 아니라 입력 자격을 통제하는 레이어에 더 가깝습니다."
  - question: "검색 품질이 좋아지면 계약 관리가 덜 중요해지지 않나요?"
    answer: "아니요. 검색 품질은 관련성이 높은 조각을 찾는 문제이고, 계약 관리는 그 조각이 지금 이 작업에 들어와도 되는지 판정하는 문제입니다. 관련성이 높아도 stale하거나 민감도 규칙을 위반하면 운영 품질은 오히려 나빠질 수 있습니다."
  - question: "도입을 어디서부터 시작하는 게 가장 현실적인가요?"
    answer: "incident 대응, 코드 수정, 외부 전송처럼 실패 비용이 큰 작업군부터 시작하는 것이 좋습니다. 각 작업에 실제로 들어가는 상위 컨텍스트 조각만 먼저 인벤토리화하고 owner, ttl, sensitivity, allowed_tasks 다섯 축만 붙여도 초반 효과가 큽니다."
operator_checklist:
  - "high-risk 작업군부터 허용 입력과 금지 입력을 표로 명시한다."
  - "각 컨텍스트 조각에 owner, freshness_ttl, sensitivity, max_tokens, allowed_tasks를 붙인다."
  - "stale context, 민감도 위반, 토큰 예산 초과를 validator에서 분리 경고한다."
  - "정책 변경은 canary 후 rollout하고 receipt로 실제 입력 조각을 남긴다."
key_takeaways:
...
### 7) 운영 회의에서는 "검색 품질"보다 "계약 위반 사례"를 먼저 리뷰하는 편이 낫다

실무에서 회의가 길어지는 이유 중 하나는 검색 정확도와 모델 답변 품질을 먼저 이야기하기 때문입니다. 물론 그것도 중요하지만, 입력 계약이 흔들리는 상태에서는 검색 튜닝이 임시 봉합으로 끝날 가능성이 큽니다. 저는 주간 리뷰에서 아래 순서로 보는 편이 훨씬 효율적이라고 생각합니다.

1. 지난주 contract violation 사례 3건
2. stale context가 실제 의사결정을 흔든 사례 3건
3. allowed_tasks 분류가 애매했던 작업군 1, 2개
4. 그다음에 retrieval 품질과 토큰 비용을 본다

이 순서를 쓰면 운영 품질 대화가 "어떤 검색기를 붙일까"에서 "왜 승인되지 않은 문맥이 들어왔나"로 바뀝니다. 이 차이가 큽니다. 에이전트 입력 문제는 대개 검색 recall 부족보다, **들어오면 안 되는 문맥이 너무 쉽게 섞이는 구조**에서 더 자주 생기기 때문입니다.

아래처럼 회의용 간단한 표를 두면 빠르게 정리됩니다.

| 점검 항목 | 질문 | 바로 필요한 액션 |
| --- | --- | --- |
| contract violation | 금지 소스 또는 TTL 초과 조각이 들어왔는가 | 규칙 보강, owner 지정 |
| stale context | 최신 문서보다 오래된 요약이 우선 사용됐는가 | freshness 기준 수정 |
| allowed_tasks drift | 다른 작업군 문맥이 섞였는가 | task 분류 재정의 |
| token efficiency | 줄인 토큰이 품질 저하를 만들었는가 | 압축 규칙 재조정 |

이 표를 기준으로 보면 registry는 추상적인 메타데이터 체계가 아니라, 운영 회의에서 바로 고칠 항목을 드러내는 관제판 역할을 하게 됩니다.
