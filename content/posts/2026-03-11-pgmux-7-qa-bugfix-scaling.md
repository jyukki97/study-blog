...
이번 편의 수정은 단순 버그 패치라기보다, 프록시를 "한 대짜리 데모"에서 "여러 대로 늘려도 버틸 수 있는 서비스"로 옮기는 작업에 가깝다. 특히 [쿼리 캐싱 편](/posts/2026-03-11-pgmux-4-query-caching/)에서 도입한 캐시는 로컬 메모리 기반이라 스케일아웃 순간 정합성 문제가 바로 드러난다. 그래서 운영에서는 **캐시 적중률보다 stale risk를 먼저 측정**해야 한다.
...
이 지점이 정리돼 있어야 이후 [Circuit Breaker와 Rate Limit](/posts/2026-03-11-pgmux-11-circuit-breaker-ratelimit/)이나 [Causal Consistency](/posts/2026-03-11-pgmux-13-lsn-causal-consistency/) 같은 상위 안정성 기능도 의미를 갖는다. 기본 정합성이 흔들리면 상위 제어 로직은 오히려 장애를 더 복잡하게 만들 수 있기 때문이다.
