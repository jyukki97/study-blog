...
module: "foundation"
quizzes:
  - question: "단일 책임 원칙(SRP)에 따르면, 클래스를 변경해야 하는 이유는 오직 몇 가지여야 할까요?"
    options:
      - "0가지 (변경하면 안 된다)"
      - "1가지"
      - "2가지 (데이터 변경, 로직 변경)"
      - "제한 없다"
    answer: 1
    explanation: "클래스는 오직 하나의 액터(Actor)나 하나의 책임에 대해서만 변경되어야 합니다. 여러 책임이 섞이면 변경의 여파가 커집니다."

  - question: "개방-폐쇄 원칙(OCP)의 핵심 내용인 '확장에는 열려 있고, 수정에는 닫혀 있다'는 것은 실제 코드로 어떻게 구현하나요?"
    options:
      - "모든 변수를 private으로 선언한다."
      - "if-else 문을 많이 사용하여 모든 경우의 수를 대비한다."
      - "인터페이스나 추상 클래스를 정의하고, 이를 상속받아 새로운 기능을 추가한다."
      - "final 키워드를 사용하여 상속을 막는다."
    answer: 2
    explanation: "기존 코드는 추상화(인터페이스)에 의존하게 하여 수정하지 않고, 새 클래스를 만들어 기능을 확장하는 방식(다형성)을 사용합니다."

  - question: "리스코프 치환 원칙(LSP)을 위반하는 대표적인 예시는?"
    options:
      - "자식 클래스가 부모 클래스의 메서드를 오버라이딩(재정의)하는 것"
      - "직사각형(Rectangle)을 상속받아 정사각형(Square)을 만들었는데, 너비를 바꾸니 높이도 바뀌어 예상치 못한 동작을 하는 경우"
      - "인터페이스를 구현하여 새로운 기능을 추가하는 경우"
      - "추상 클래스의 생성자를 호출하는 경우"
    answer: 1
    explanation: "자식 클래스(Square)가 부모(Rectangle)의 '너비와 높이는 독립적'이라는 기대 동작(계약)을 깨뜨렸기 때문에 LSP 위반입니다."

  - question: "인터페이스 분리 원칙(ISP)이 해결하고자 하는 문제는?"
    options:
      - "인터페이스 이름이 너무 긴 문제"
      - "자신이 사용하지 않는 메서드까지 억지로 구현해야 하는 '비대한 인터페이스' 문제"
      - "인터페이스가 너무 잘게 쪼개져 있는 문제"
      - "다중 상속이 불가능한 문제"
    answer: 1
    explanation: "필요 없는 메서드에 의존하면, 그 메서드가 변경될 때 불필요한 영향을 받습니다. 인터페이스는 클라이언트가 필요한 만큼만 작게 쪼개야 합니다."

  - question: "의존성 역전 원칙(DIP)에서 말하는 '상위 모듈이 하위 모듈에 의존하면 안 된다'는 말의 실무적인 의미는?"
    options:
      - "Controller가 Service를 호출하면 안 된다."
      - "비즈니스 로직(Service)이 구체적인 구현체(예: MySQLRepository)가 아니라, 추상화(Repository 인터페이스)에 의존해야 한다."
      - "하위 모듈을 먼저 개발해야 한다."
      - "의존성을 아예 없애야 한다."
    answer: 1
    explanation: "구체적인 기술(DB, Framework)이 바뀌어도 핵심 비즈니스 로직은 영향을 받지 않도록, 중간에 인터페이스(추상화)를 두라는 의미입니다."

  - question: "다음 중 SOLID 원칙을 잘 지킨 설계의 특징은?"
    options:
      - "하나의 거대한 클래스에 모든 기능이 모여 있다."
      - "새로운 기능을 추가할 때 기존 코드를 거의 수정하지 않는다."
      - "모든 클래스가 서로 강하게 결합되어 있다."
      - "인터페이스 없이 구체 클래스끼리 직접 의존한다."
    answer: 1
    explanation: "높은 응집도(Cohesion)와 낮은 결합도(Coupling)를 유지하여, 변경 사항이 발생해도 유연하게 대처할 수 있습니다."

  - question: "Spring Framework의 의존성 주입(Dependency Injection, DI)은 SOLID 중 어떤 원칙과 가장 밀접한가요?"
    options:
      - "SRP"
      - "OCP"
      - "LSP"
      - "DIP"
    answer: 3
    explanation: "DI 컨테이너가 객체 의존성을 외부에서 주입해줌으로써, 객체가 구체 클래스 대신 추상화(DIP)에 의존하면서도 동작하게 만듭니다."

  - question: "단일 책임 원칙 위반을 의심해볼 수 있는 신호(Code Smell)는?"
    options:
      - "클래스 이름이 명확하다."
      - "메서드 개수가 적다."
      - "클래스 내에서 `User`, `Email`, `Payment`, `Log` 등 서로 관련 없는 여러 도메인의 로직이 섞여 있다."
      - "인터페이스를 구현하고 있다."
    answer: 2
    explanation: "서로 다른 목적(Actor)의 코드가 섞여 있으면, 한 쪽의 변경이 다른 쪽에 영향을 미치게 됩니다."
study_order: 30
---
