...
quizzes:
  - question: "Kafka 소비 실패를 '일시적(transient) 실패'와 '영구(permanent) 실패'로 분류할 때, 영구 실패의 예시로 적절한 것은?"
    options:
      - "외부 API의 일시적인 타임아웃"
      - "데이터베이스 연결 장애"
      - "메시지 스키마 오류 또는 필수 필드 누락 (코드/데이터 수정 없이는 해결 불가)"
      - "네트워크 일시 불안정"
    answer: 2
    explanation: "영구 실패는 재시도해도 해결되지 않는 오류입니다. 잘못된 스키마나 비즈니스 규칙 위반은 코드나 데이터를 수정해야만 해결됩니다. 이러한 메시지는 빠르게 DLQ로 분리해야 합니다."

  - question: "실패한 메시지를 원본 토픽 내에서 스레드를 멈추고 즉시 반복 재시도할 때 발생할 수 있는 문제점은?"
    options:
      - "메시지 처리 순서가 뒤바뀔 수 있다."
      - "해당 파티션 전체의 처리가 막혀 Lag이 급증하고 전체 처리량이 급격히 저하된다."
      - "메시지가 자동으로 삭제된다."
      - "다른 Consumer Group에도 영향을 준다."
    answer: 1
    explanation: "즉시 재시도는 해당 파티션을 점유한 상태로 대기하므로, 뒤따르는 모든 정상 메시지의 처리도 막히게 됩니다. 이를 방지하려면 실패 메시지를 별도의 재시도 토픽으로 분리해야 합니다."

  - question: "재시도 토픽(Retry Topic) 패턴의 가장 큰 장점은?"
    options:
      - "구현이 가장 간단하다."
      - "재시도 대상 메시지를 분리하여 원본 파티션의 처리 흐름을 막지 않는다."
      - "메시지가 절대 유실되지 않는다."
      - "자동으로 멱등 처리가 보장된다."
    answer: 1
    explanation: "재시도 토픽 패턴은 실패 메시지를 별도 토픽으로 옮겨 지연 후 재처리하므로, 원본 토픽 Consumer는 다음 메시지를 계속 처리할 수 있어 전체 처리량 저하를 방지합니다."

  - question: "DLQ(Dead Letter Queue)의 올바른 운영 방식은?"
    options:
      - "DLQ에 도착한 메시지는 영원히 무시해도 된다."
      - "DLQ 유입량을 모니터링하고, 원인 분석 후 코드/데이터 수정 뒤 재처리(Replay)하는 운영 루프를 구축해야 한다."
      - "DLQ는 디스크 공간 절약을 위해 자동 삭제 정책을 설정해야 한다."
      - "DLQ에 메시지가 쌓이면 파티션 수를 늘려야 한다."
    answer: 1
    explanation: "DLQ는 '메시지 무덤'이 아니라 '장애 분석 및 재처리의 시작점'입니다. 알람을 설정하고, 원인을 분석하여 문제 해결 후 재처리할 수 있는 도구와 프로세스가 필요합니다."

  - question: "Kafka Consumer 환경에서 '멱등(Idempotent) 처리'가 필요한 이유로 가장 적절한 것은?"
    options:
      - "메시지 순서가 보장되지 않기 때문에"
      - "리밸런싱이나 at-least-once 커밋 전략으로 인해 동일 메시지가 중복 소비될 수 있기 때문에"
      - "Producer가 메시지를 중복 전송하는 것을 막기 위해"
      - "Consumer의 처리 속도를 높이기 위해"
    answer: 1
    explanation: "대부분의 Kafka Consumer는 at-least-once 방식으로 동작하며, 처리 후 커밋 전에 장애가 나면 같은 메시지를 다시 소비합니다. 따라서 비즈니스 로직이 중복 실행되어도 결과가 한 번만 반영되도록(멱등성) 설계해야 합니다."
study_order: 402
---
