...
- IoC/DI, Bean 생명주기, 프록시 구조
- 트랜잭션 경계/전파/롤백 규칙
- Validation/Exception 규약, API 계약
- Spring Boot 자동설정/프로필 분리
- 테스트 전략(단위/슬라이스/통합)

## 미니 실습

- **Self-invocation 재현**: 내부 메서드 호출 시 트랜잭션이 안 먹는 케이스 만들기
- **Validation 응답 규약**: 필드 에러/비즈니스 에러 구분한 공통 응답 만들기
- **프로필 분리**: local/stage/prod 설정을 분리하고 동작 확인
